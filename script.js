...
webtoonCategories.forEach(c => {
  c.list.forEach(name => {
    ...
    if(!cleanWebtoonList.includes(cleanName)) {
        cleanWebtoonList.push(cleanName);
    }
  });
});
...
window.saveAllData = async function() {
  ...
  try {
    ...
    const dbRef = doc(window.db, "ti_me_data", "my_shared_data");
    await setDoc(dbRef, { 
        projects, wishList, scrapList, compareLogs, deletedCands, taggedWorksData, tagCategories, readWorksList, readingBooks 
    });
  } catch (error) { 
...
function saveReadingLogData() { saveAllData(); }
...
setInterval(() => { 
  if (!isDragging && usagi) { 
      usagi.src = `usagi${Math.floor(Math.random() * 4) + 1}.gif`; 
  } 
}, 10000);
...
function getNextTitle(baseTitle) {
  ...
  let count = 2; 
  while (existingTitles.includes(`${baseTitle} ${count}`)) { 
      count++; 
  } 
  ...
  tagCategories.forEach(category => {
    category.items.forEach(name => {
      projects[id].items.push({ 
          ...
          name: name, memo: '', img: null, 
          zone: category.type === 'genre' ? 'pool-genre' : (category.type === 'top' ? 'pool-top' : (category.type === 'bottom' ? 'pool-bottom' : 'pool-plot')), 
          color: category.colorClass.replace('kw-', 'bg-') 
      });
    });
  });
  saveData(); 
  ...
  webtoonCategories.forEach(category => {
    category.list.forEach(name => {
      projects[id].items.push({ 
          ...
          memo: '', img: null, 
          zone: category.zoneId, color: category.color 
      });
    });
  });
  saveData(); 
...
function renderItems() {
  document.querySelectorAll('.tier-items, .pool, .ranking-list').forEach(el => { 
      if(!el.classList.contains('cat-list')) el.innerHTML = ''; 
  });
  ...
  projects[currentId].items.forEach(item => {
    ...
    itemEl.addEventListener('dragend', function() {
      ...
      const newItems = [];
      document.querySelectorAll('#workspace-screen .item').forEach(el => { 
    ...
    const dropZone = document.querySelector(`#workspace-screen [data-zone="${item.zone}"]`); 
...
document.getElementById('btn-open-wishlist').addEventListener('click', () => { 
  hideAllScreens(); 
  document.getElementById('wishlist-screen').style.display = 'block'; 
  renderWishList(); 
});
...
window.addWishItem = function() {
  ...
  wishList.push({ id: Date.now(), name: input.value.trim(), checked: false }); 
  input.value = ''; 
  saveWish(); 
  renderWishList();
}
...
document.getElementById('btn-open-scrap').addEventListener('click', () => { 
  hideAllScreens(); 
  document.getElementById('scrap-screen').style.display = 'block'; 
  renderScrapList(); 
});
...
window.addScrapItem = function() {
  ...
  if(file) { 
      ...
      reader.onload = (e) => { 
          newScrap.img = e.target.result; 
          scrapList.unshift(newScrap); 
          saveScraps(); 
          renderScrapList(); 
      }; 
  ...
  document.getElementById('scrap-comment').value = ''; 
  document.getElementById('scrap-image').value = '';
}
...
function renderScrapList() {
  ...
  scrapList.forEach(s => {
    ...
    card.innerHTML = `
      ${s.img ? `<img src="${s.img}" class="card-img">` : ''} 
      <button class="scrap-del-btn" onclick="deleteScrap(${s.id})">✕</button> 
      <div class="card-content"> 
          ${s.url ? `<a href="${s.url}" target="_blank" class="card-url">🔗 ${s.url}</a>` : ''} 
          <div class="card-comment">${s.comment}</div> 
      </div>`; 
...
window.deleteScrap = function(id) { 
  if(confirm("이 스크랩을 지울까요?")) { 
      scrapList = scrapList.filter(s => s.id !== id); 
      saveScraps(); 
      renderScrapList(); 
  } 
}
...
window.openCategoryRank = function(tierId) {
  ...
  if(tierItems.length < 2) return alert("작품이 2개 이상 있어야 합니다!");
  
  hideAllScreens(); 
  document.getElementById('category-rank-screen').style.display = 'block'; 
  document.getElementById('cat-rank-title').innerText = `[${tierId}] 세부 순위`;
  
  ['art', 'story', 'scene'].forEach(cat => {
    const container = document.getElementById(`cat-${cat}-list`); 
    container.innerHTML = '';
    ...
    let sortedItems = [...tierItems].sort((a, b) => { 
        let idxA = savedOrder.indexOf(a.itemId); 
        let idxB = savedOrder.indexOf(b.itemId); 
        return (idxA === -1 ? 999 : idxA) - (idxB === -1 ? 999 : idxB); 
    });
    
    sortedItems.forEach(item => {
      const el = document.createElement('div'); 
      el.className = `item ${item.color || ''} cat-sort-item`; 
      el.draggable = true; 
...
function saveCategoryRanks(tierId) {
  ...
  ['art', 'story', 'scene'].forEach(cat => { 
      ...
      const items = [...zone.querySelectorAll('.cat-sort-item')]; 
...
function updateWcUI() {
  ...
  if (wcMatchIndex >= wcCurrentRound.length - 1) { 
      wcNextRound.push(wcCurrentRound[wcMatchIndex]); 
      wcRankings.unshift([...wcLosersThisRound]); 
      wcLosersThisRound = []; 
...
window.openTierCompare = function(tierId) {
  ...
  document.getElementById('comp-title').innerText = `[${tierId}] 티어 1:1 비교소`; 
  compareLogs = []; 
  saveLogs(); 
...
document.getElementById('btn-compare').addEventListener('click', () => { 
    currentCompareTier = null; 
    document.getElementById('btn-apply-rank').style.display = 'none'; 
...
function openCompareMode(itemList) {
  ...
  document.getElementById('comp-fixed').innerHTML = '우클릭 하세요'; 
  document.getElementById('comp-fixed').classList.add('empty'); 
  document.getElementById('comp-target').innerHTML = '좌클릭 하세요'; 
  ...
  itemList.forEach(name => {
    ...
    wrapper.className = 'cand-wrapper';
    ...
    btn.onclick = () => { 
        document.getElementById('comp-target').innerText = name; 
        document.getElementById('comp-target').classList.remove('empty');
    };
    btn.oncontextmenu = (e) => { 
        e.preventDefault(); 
        document.getElementById('comp-fixed').innerText = name; 
        document.getElementById('comp-fixed').classList.remove('empty');
    };
    
    const delBtn = document.createElement('button'); 
    delBtn.className = 'cand-del-btn'; 
    delBtn.innerText = '✕';
    delBtn.onclick = () => { 
        if(confirm(`영구 삭제할까요?`)) { 
            deletedCands.push(name); 
            saveDeletedCands(); 
            wrapper.remove(); 
        } 
    };
...
window.recordCompare = function(side) {
  ...
  let winner = (side === 'fixed') ? fixed : target; 
  let loser = (side === 'fixed') ? target : fixed;
...
function renderLogs() {
  ...
  if (compareLogs.length === 0) { 
      logArea.innerHTML = '<span style="color:#999;">기록이 없습니다.</span>'; 
  } else {
    logArea.innerHTML = ''; 
...
function renderAnalysis() {
  ...
  if (compareLogs.length === 0) { 
      analysisArea.innerHTML = '<span style="color:#999;">기록을 쌓으면 엘로(Elo) 랭킹이 분석됩니다.</span>'; 
      currentRankArr = []; 
      return; 
  }
  ...
  function getElo(name) { 
      if (!eloStats[name]) eloStats[name] = { rating: 1000, wins: 0, losses: 0 }; 
      return eloStats[name]; 
  }
  
  [...compareLogs].reverse().forEach(log => {
    if(log.winner && log.loser) {
      let pWinner = getElo(log.winner); 
      let pLoser = getElo(log.loser);
      ...
      pWinner.rating = pWinner.rating + K * (1 - expectedWinRateWinner); 
      pLoser.rating = pLoser.rating + K * (0 - expectedWinRateLoser); 
      pWinner.wins++; 
      pLoser.losses++;
    }
  });
  
  let rankArr = Object.keys(eloStats).map(name => { 
      return { name: name, rating: Math.round(eloStats[name].rating), wins: eloStats[name].wins, losses: eloStats[name].losses }; 
  });
  
  rankArr.sort((a, b) => b.rating - a.rating); 
  currentRankArr = rankArr; 
...
window.deleteLog = function(id) { 
    compareLogs = compareLogs.filter(l => l.id !== id); 
    saveLogs(); 
    renderLogs(); 
}
...
window.applyRankingToTier = function() {
  if(!currentCompareTier) return; 
  if(currentRankArr.length === 0) return alert("승리 버튼을 눌러 순위를 정해주세요!");
  ...
  tierItems.sort((a, b) => { 
      let idxA = sortedNames.indexOf(a.name); 
      let idxB = sortedNames.indexOf(b.name); 
      return (idxA === -1 ? 999 : idxA) - (idxB === -1 ? 999 : idxB); 
  });
  
  projects[currentId].items = [...otherItems, ...tierItems]; 
  saveData(); 
  renderItems(); 
  alert(`결과 박스 안에 1위부터 자동 정렬되었습니다!`); 
...
document.getElementById('btn-confirm-add-work').addEventListener('click', () => {
  ...
  taggedWorksData.unshift(newWork); 
  saveTaggingData(); 
  renderTaggingGrid(); 
...
window.deleteTaggingWork = function() {
  ...
  if(confirm(`'${work.title}' 작품을 서재에서 완전히 삭제할까요?`)) { 
...
function renderTaggingGrid() {
  ...
  taggedWorksData.forEach(work => {
    ...
    square.innerHTML = `
      <button class="del-tag" onclick="event.stopPropagation(); deleteTaggingWorkGrid('${work.id}')">✕</button>
...
window.deleteTaggingWorkGrid = function(workId) {
  ...
  if(confirm(`'${work.title}' 작품을 서재에서 완전히 삭제할까요?`)) { 
      taggedWorksData = taggedWorksData.filter(w => w.id !== workId); 
      saveTaggingData(); 
      renderTaggingGrid(); 
  }
}
...
window.deleteKeywordFromPool = function(catIndex, kwIndex) {
  ...
  if(confirm(`'${targetWord}' 키워드를 삭제할까요?`)) { 
...
function renderReadWorks() {
  ...
  readWorksList.forEach((work, index) => {
    if (work.colorClass === 'bg-white' || work.platform === '봄툰 등' || !work.colorClass) {
      work.colorClass = 'bg-yellow-light'; 
      work.platform = '봄툰';
      webtoonCategories.forEach(cat => {
    ...
    el.innerHTML = `
      ...
      <div class="work-col-actions">
        <button onclick="event.stopPropagation(); deleteReadWork(${index})">삭제</button>
      </div>
    `;
...
window.renderReadingBooks = function() {
  const container = document.getElementById('reading-book-grid'); 
  container.innerHTML = '';
  ...
  readingBooks.forEach(book => {
    ...
    card.innerHTML = `
      <div class="book-actions">
          ...
          <button class="btn-book-del" onclick="event.stopPropagation(); deleteReadingBook(${book.id})" title="삭제">✕</button>
      </div>
...
window.deleteReadingEntry = function(id) { 
...
window.renderReadingEntries = function() {
  const container = document.getElementById('reading-entry-list'); 
  container.innerHTML = '';
  ...
  if (order === 'latest') { 
      sortedEntries.sort((a, b) => b.id - a.id); 
  } else { 
